...
expenses_record_bp = Blueprint("expenses_record", __name__)

@expenses_record_bp.route("/expenses_record", methods=["GET"])
@jwt_required()
def get_expenses_record():
    ...
    expenses_record = [{"id": row.id, "description": row.description, "amount": float(row.amount), "date": str(row.date)} for row in rows]

    if DEBUG:
        print(f"Fetched expenses_record: {expenses_record}")  # Debug log

    return jsonify({"expenses_record": expenses_record}), 200

@expenses_record_bp.route("/expenses_record/<int:expense_id>", methods=["DELETE"])
@jwt_required()
def delete_expense_record(expense_id):
    user_id = int(get_jwt_identity())
    query = """
        DELETE FROM common_expenses_record
        WHERE id = :expense_id
...
@expenses_record_bp.route("/expenses_record", methods=["POST"])
@jwt_required()
def post_expenses_record():
    user_id = int(get_jwt_identity())
...
@expenses_record_bp.route("/expenses_record/<int:id>", methods=["PATCH"])
@jwt_required()
def patch_expenses_record(id):
    user_id = int(get_jwt_identity())
    data = request.get_json()
    ...
    query = (f"UPDATE common_expenses_record SET {set_clause} "
             f"WHERE id = :id"
    ...
    return {"message": "updated expenses_record"}, 200